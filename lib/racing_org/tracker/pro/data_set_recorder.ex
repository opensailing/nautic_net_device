defmodule RacingOrg.Tracker.Pro.DataSetRecorder do
  @moduledoc """
  ...
  See also: RacingOrg.Tracker.Pro.DataSetUploader
  """
  use GenServer
  ...
  defp save_data_points(data_points, state) do
    data_set = RacingOrg.Tracker.Pro.data_set(data_points)
    ...
    RacingOrg.Tracker.Pro.Race.Archive.record(encoded)
    DataSetUploader.add_file(path)
  end
  ...
  defp save_network_devices(network_devices, state) do
    data_set = RacingOrg.Tracker.Pro.data_set([], network_devices: network_devices)
