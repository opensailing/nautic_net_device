defmodule RacingOrg.Tracker.Pro.DesiredState.Manager do
  ...
  defp with_session_in_manager(session_holder, session_generation, fun)
    ...
    case SessionBridge.callback_principal(session_holder) do
      {:ok, callback_pid} ->
        manager_pid = self()
      ...
      {:error, :session_callback_failed} = error ->
        error
    end
  end

  defp await_session_callback(
         worker_pid,
         monitor_ref,
         request_token,
    ...
    receive do
      ...
      when is_pid(callback_pid) and is_reference(callback_request_token) ->
        if SessionBridge.valid_callback_request?(
             self(),
             request_token,
             expected_callback_pid,
             callback_pid,
          ...
          callback_result = invoke_session_callback(fun, session)
          ...
          try do
            await_session_callback_result(
              worker_pid,
              monitor_ref,
              request_token
            )
          after
            AuthorityRequest.delete(result_token)
          end
        else
          await_session_callback(
            worker_pid,
      ...
      {:session_callback_complete, ^request_token, ^worker_pid, completion_token, result}
      when is_reference(completion_token) ->
        if SessionBridge.valid_completion?(
             self(),
             request_token,
             worker_pid,
             completion_token,
  ...
  defp await_session_callback_result(
         worker_pid,
         monitor_ref,
         request_token
       ) do
  ...
  defp acknowledge_session_callback(
         worker_pid,
         monitor_ref,
         request_token,
         result
    ...
    after
      AuthorityRequest.delete(ack_token)
    end
  end

  defp invoke_session_callback(fun, session) do
    {:ok, fun.(session)}
  rescue
    _exception -> {:error, :session_callback_failed}
  catch
    _kind, _reason -> {:error, :session_callback_failed}
  end
